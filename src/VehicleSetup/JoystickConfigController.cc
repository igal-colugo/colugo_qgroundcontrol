...
JoystickConfigController::JoystickConfigController(void) : _joystickManager(qgcApp()->toolbox()->joystickManager())
{
...
bool JoystickConfigController::getDeadbandToggle()
{
    ...
    if (_activeJoystick)
    {
        returnValue = _activeJoystick->deadband();
    }
...
void JoystickConfigController::_inputCenterWaitBegin(Joystick::AxisFunction_t function, int axis, int value)
{
    ...
    {
        // sensing deadband
        if ((abs(value) * 1.1f > _rgAxisInfo[axis].deadband) && (_activeJoystick->deadband()))
...
void JoystickConfigController::_inputStickDetect(Joystick::AxisFunction_t function, int axis, int value)
{
    ...
    else if (axis == _stickDetectAxis)
    {
        if (_stickSettleComplete(axis, value))
        {
            ...
            qCDebug(JoystickConfigControllerLog) << "_inputStickDetect saving values, function:axis:value:reversed:_axisValueSave" << function << axis << value << info->reversed
                                                 << _axisValueSave[axis];
            _signalAllAttitudeValueChanges();
            _advanceState();
        }
...
void JoystickConfigController::_setInternalCalibrationValuesFromSettings()
{
    ...
    for (int axis = 0; axis < _axisCount; axis++)
    {
        ...
        qCDebug(JoystickConfigControllerLog) << "Read settings name:axis:min:max:trim:reversed" << joystick->name() << axis << info->axisMin << info->axisMax << info->axisTrim
                                             << info->reversed;
    }
