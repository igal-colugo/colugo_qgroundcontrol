...
GPSManager::GPSManager(QGCApplication *app, QGCToolbox *toolbox) : QGCTool(app, toolbox)
{
...
void GPSManager::connectGPS(const QString &device, const QString &gps_type)
{
    ...
    _gpsProvider = new GPSProvider(device, type, true, /* enableSatInfo */
                                   rtkSettings->surveyInAccuracyLimit()->rawValue().toDouble(), rtkSettings->surveyInMinObservationDuration()->rawValue().toInt(),
                                   rtkSettings->useFixedBasePosition()->rawValue().toBool(), rtkSettings->fixedBasePositionLatitude()->rawValue().toDouble(),
                                   rtkSettings->fixedBasePositionLongitude()->rawValue().toDouble(), rtkSettings->fixedBasePositionAltitude()->rawValue().toFloat(),
