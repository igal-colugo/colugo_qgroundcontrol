...
class CommtactUDPCLient
{
  public:
    CommtactUDPCLient(const QHostAddress &address_, quint16 port_) : address(address_), port(port_)
    {
    }
    CommtactUDPCLient(const CommtactUDPCLient *other) : address(other->address), port(other->port)
    {
    }
...
class CommtactUDPConfiguration : public CommtactLinkConfiguration
{
    Q_OBJECT
  ...
  public:
    ...
    CommtactUDPConfiguration(const QString &name);
    CommtactUDPConfiguration(CommtactUDPConfiguration *source);
    ~CommtactUDPConfiguration();
    ...
    const QList<CommtactUDPCLient *> targetHosts(void)
    {
        return _targetHosts;
    }
    ...
    {
        return CommtactLinkConfiguration::TypeUdp;
    }
  ...
  private:
    ...
    void _copyFrom(CommtactLinkConfiguration *source);

    QList<CommtactUDPCLient *> _targetHosts;
    QStringList _hostList;
    quint16 _localPort;
};

class CommtactUDPLink : public CommtactLinkInterface
{
    Q_OBJECT

  public:
    CommtactUDPLink(SharedCommtactLinkConfigurationPtr &config);
    virtual ~CommtactUDPLink();
  ...
  private:
    ...
    void _writeDataGram(const QByteArray data, const CommtactUDPCLient *target);

    bool _running;
    QUdpSocket *_socket;
    CommtactUDPConfiguration *_udpConfig;
    bool _connectState;
    QList<CommtactUDPCLient *> _sessionTargets;
