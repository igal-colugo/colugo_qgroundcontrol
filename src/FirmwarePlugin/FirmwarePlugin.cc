...
QString FirmwarePlugin::flightMode(uint8_t base_mode, uint32_t custom_mode) const
{
    ...
    static const struct Bit2Name rgBit2Name[] = {
        {MAV_MODE_FLAG_MANUAL_INPUT_ENABLED, "Manual"}, {MAV_MODE_FLAG_STABILIZE_ENABLED, "Stabilize"}, {MAV_MODE_FLAG_GUIDED_ENABLED, "Guided"},
        {MAV_MODE_FLAG_AUTO_ENABLED, "Auto"},           {MAV_MODE_FLAG_TEST_ENABLED, "Test"},
    };
...
const QVariantList &FirmwarePlugin::toolIndicators(const Vehicle *)
{
    ...
    if (_toolIndicatorList.size() == 0)
    {
        _toolIndicatorList = QVariantList({
            ...
            QVariant::fromValue(QUrl::fromUserInput("qrc:/toolbar/LinkStatusIndicator.qml")),
...
void FirmwarePlugin::checkIfIsLatestStable(Vehicle *vehicle)
{
    ...
    connect(downloader, &QGCFileDownload::downloadComplete, this, [vehicle, this](QString remoteFile, QString localFile, QString errorMsg) {
        if (errorMsg.isEmpty())
        {
...
void FirmwarePlugin::_versionFileDownloadFinished(QString &remoteFile, QString &localFile, Vehicle *vehicle)
{
    ...
    {
        QString currentVersionNumber = QString("%1.%2.%3").arg(vehicle->firmwareMajorVersion()).arg(vehicle->firmwareMinorVersion()).arg(vehicle->firmwarePatchVersion());
...
int FirmwarePlugin::versionCompare(Vehicle *vehicle, int major, int minor, int patch)
{
    ...
    if (currMajor > major || (currMajor == major && currMinor > minor) || (currMajor == major && currMinor == minor && currPatch > patch))
    {
        return 1;
    }
    return -1;
...
void FirmwarePlugin::sendGCSMotionReport(Vehicle *vehicle, FollowMe::GCSMotionReport &motionReport, uint8_t estimationCapabilities)
{
    ...
    if (!weakLink.expired())
    {
        ...
        mavlink_msg_follow_target_encode_chan(static_cast<uint8_t>(mavlinkProtocol->getSystemId()), static_cast<uint8_t>(mavlinkProtocol->getComponentId()), sharedLink->mavlinkChannel(),
