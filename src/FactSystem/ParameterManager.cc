...
const QHash<int, QString> _mavlinkCompIdHash{
    {MAV_COMP_ID_CAMERA, "Camera1"},  {MAV_COMP_ID_CAMERA2, "Camera2"}, {MAV_COMP_ID_CAMERA3, "Camera3"}, {MAV_COMP_ID_CAMERA4, "Camera4"}, {MAV_COMP_ID_CAMERA5, "Camera5"},
    {MAV_COMP_ID_CAMERA6, "Camera6"}, {MAV_COMP_ID_SERVO1, "Servo1"},   {MAV_COMP_ID_SERVO2, "Servo2"},   {MAV_COMP_ID_SERVO3, "Servo3"},   {MAV_COMP_ID_SERVO4, "Servo4"},
    {MAV_COMP_ID_SERVO5, "Servo5"},   {MAV_COMP_ID_SERVO6, "Servo6"},   {MAV_COMP_ID_SERVO7, "Servo7"},   {MAV_COMP_ID_SERVO8, "Servo8"},   {MAV_COMP_ID_SERVO9, "Servo9"},
    {MAV_COMP_ID_SERVO10, "Servo10"}, {MAV_COMP_ID_SERVO11, "Servo11"}, {MAV_COMP_ID_SERVO12, "Servo12"}, {MAV_COMP_ID_SERVO13, "Servo13"}, {MAV_COMP_ID_SERVO14, "Servo14"},
    {MAV_COMP_ID_GIMBAL, "Gimbal1"},  {MAV_COMP_ID_ADSB, "ADSB"},       {MAV_COMP_ID_OSD, "OSD"},         {MAV_COMP_ID_FLARM, "FLARM"},     {MAV_COMP_ID_GIMBAL2, "Gimbal2"},
    {MAV_COMP_ID_GIMBAL3, "Gimbal3"}, {MAV_COMP_ID_GIMBAL4, "Gimbal4"}, {MAV_COMP_ID_GIMBAL5, "Gimbal5"}, {MAV_COMP_ID_GIMBAL6, "Gimbal6"}, {MAV_COMP_ID_IMU, "IMU1"},
...
ParameterManager::ParameterManager(Vehicle *vehicle)
    : QObject(vehicle), _vehicle(vehicle), _mavlink(nullptr), _loadProgress(0.0), _parametersReady(false), _missingParameters(false), _initialLoadComplete(false),
      _waitingForDefaultComponent(false), _saveRequired(false), _metaDataAddedToFacts(false),
      _logReplay(!vehicle->vehicleLinkManager()->primaryLink().expired() && vehicle->vehicleLinkManager()->primaryLink().lock()->isLogReplay()), _prevWaitingReadParamIndexCount(0),
      _prevWaitingReadParamNameCount(0), _prevWaitingWriteParamNameCount(0), _initialRequestRetryCount(0), _disableAllRetries(false), _indexBatchQueueActive(false), _totalParamCount(0)
{
...
void ParameterManager::_handleParamValue(int componentId, QString parameterName, int parameterCount, int parameterIndex, MAV_PARAM_TYPE mavParamType, QVariant parameterValue)
{
    ...
    qCDebug(ParameterManagerVerbose1Log) << _logVehiclePrefix(componentId) << "_parameterUpdate"
                                         << "name:" << parameterName << "count:" << parameterCount << "index:" << parameterIndex << "mavType:" << mavParamType << "value:" << parameterValue
                                         << ")";
    ...
    if (!_waitingReadParamIndexMap[componentId].contains(parameterIndex) && !_waitingReadParamNameMap[componentId].contains(parameterName) &&
...
void ParameterManager::refreshAllParameters(uint8_t componentId)
{
    ...
    mavlink_msg_param_request_list_pack_chan(mavlink->getSystemId(), mavlink->getComponentId(), sharedLink->mavlinkChannel(), &msg, _vehicle->id(), componentId);
    ...
    for (int componentId : _waitingReadParamIndexMap.keys())
    {
        ...
        for (int paramIndex : _waitingReadParamIndexMap[componentId].keys())
        {
            ...
            if (_disableAllRetries || _waitingReadParamIndexMap[componentId][paramIndex] > _maxInitialLoadRetrySingleParam)
            {
                ...
                qCDebug(ParameterManagerLog) << _logVehiclePrefix(componentId) << "Giving up on (paramIndex:" << paramIndex
            ...
            else
            {
                ...
                qCDebug(ParameterManagerLog) << _logVehiclePrefix(componentId) << "Read re-request for (paramIndex:" << paramIndex
                                             << "retryCount:" << _waitingReadParamIndexMap[componentId][paramIndex] << ")";
            }
        }
    }
        ...
        for (int componentId : _waitingWriteParamNameMap.keys())
        {
            for (const QString &paramName : _waitingWriteParamNameMap[componentId].keys())
            {
                ...
                if (_waitingWriteParamNameMap[componentId][paramName] <= _maxReadWriteRetry)
                {
                    ...
                    qCDebug(ParameterManagerLog) << _logVehiclePrefix(componentId) << "Write resend for (paramName:" << paramName
    ...
    if (!paramsRequested)
    {
        for (int componentId : _waitingReadParamNameMap.keys())
        {
            for (const QString &paramName : _waitingReadParamNameMap[componentId].keys())
            {
                ...
                {
                    _readParameterRaw(componentId, paramName, -1);
                    qCDebug(ParameterManagerLog) << _logVehiclePrefix(componentId) << "Read re-request for (paramName:" << paramName
...
void ParameterManager::_sendParamSetToVehicle(int componentId, const QString &paramName, FactMetaData::ValueType_t valueType, const QVariant &value)
{
    ...
    if (!weakLink.expired())
    {
        ...
        mavlink_message_t msg;
        mavlink_msg_param_set_encode_chan(_mavlink->getSystemId(), _mavlink->getComponentId(), sharedLink->mavlinkChannel(), &msg, &p);
        _vehicle->sendMessageOnLinkThreadSafe(sharedLink.get(), msg);
    }
}

void ParameterManager::sendParamSetToVehicle(int componentId, const QString &paramName, int typeOfValue, const QVariant &value)
{
    FactMetaData::ValueType_t valueType = (FactMetaData::ValueType_t) typeOfValue;
    WeakLinkInterfacePtr weakLink = _vehicle->vehicleLinkManager()->primaryLink();
...
void ParameterManager::_tryCacheHashLoad(int vehicleId, int componentId, QVariant hash_value)
{
    ...
    if (crc32_value == hash_value.toUInt())
    {
        ...
        connect(ani, &QVariantAnimation::valueChanged, this, [this](const QVariant &value) { _setLoadProgress(value.toDouble()); });

        // Hide 500ms after animation finishes
        connect(ani, &QVariantAnimation::finished, this, [this] { QTimer::singleShot(500, [this] { _setLoadProgress(0); }); });
...
void ParameterManager::writeParametersToStream(QTextStream &stream)
{
    ...
    stream << "# Version: " << _vehicle->firmwareMajorVersion() << "." << _vehicle->firmwareMinorVersion() << "." << _vehicle->firmwarePatchVersion() << " "
    ...
    for (int componentId : _mapCompId2FactMap.keys())
    {
        for (const QString &paramName : _mapCompId2FactMap[componentId].keys())
        {
            ...
            if (fact)
            {
                stream << _vehicle->id() << "\t" << componentId << "\t" << paramName << "\t" << fact->rawValueStringFullPrecision() << "\t"
