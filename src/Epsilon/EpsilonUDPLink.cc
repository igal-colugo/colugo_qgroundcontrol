...
static bool contains_target(const QList<EpsilonUDPCLient *> list, const QHostAddress &address, quint16 port)
{
    for (int i = 0; i < list.count(); i++)
    {
        EpsilonUDPCLient *target = list[i];
...
EpsilonUDPLink::EpsilonUDPLink(SharedEpsilonLinkConfigurationPtr &config)
    : EpsilonLinkInterface(config), _running(false), _socket(nullptr), _udpConfig(qobject_cast<EpsilonUDPConfiguration *>(config.get())), _connectState(false)
{
    if (!_udpConfig)
    {
...
bool EpsilonUDPLink::_isIpLocal(const QHostAddress &add)
{
...
void EpsilonUDPLink::_writeBytes(const QByteArray data)
{
    if (!_socket)
    {
        return;
    ...
    {
        EpsilonUDPCLient *target = _udpConfig->targetHosts()[i];
    ...
    for (EpsilonUDPCLient *target : _sessionTargets)
    {
        _writeDataGram(data, target);
    }
}

void EpsilonUDPLink::_writeDataGram(const QByteArray data, const EpsilonUDPCLient *target)
{
...
void EpsilonUDPLink::disconnect(void)
{
    _running = false;
    quit();
    wait();
    if (_socket)
    {
        ...
        QObject::disconnect(_socket, &QUdpSocket::readyRead, this, &EpsilonUDPLink::readBytes);
...
void EpsilonUDPLink::testSend(void)
{
    QByteArray data("\x49\x49\x2A", 3);
    _writeBytes(data);
}

bool EpsilonUDPLink::_connect(void)
{
...
bool EpsilonUDPLink::_hardwareConnect()
{
    if (_socket)
    {
        delete _socket;
...
#endif
        QObject::connect(_socket, &QUdpSocket::readyRead, this, &EpsilonUDPLink::readBytes);
        emit connected();
    }
    else
    {
...
bool EpsilonUDPLink::isConnected() const
{
    return _connectState;
}

void EpsilonUDPLink::_registerZeroconf(uint16_t port, const std::string &regType)
{
...
EpsilonUDPConfiguration::EpsilonUDPConfiguration(EpsilonUDPConfiguration *source) : EpsilonLinkConfiguration(source)
{
    _copyFrom(source);
}

EpsilonUDPConfiguration::~EpsilonUDPConfiguration()
{
    _clearTargetHosts();
}

void EpsilonUDPConfiguration::copyFrom(EpsilonLinkConfiguration *source)
{
    EpsilonLinkConfiguration::copyFrom(source);
    _copyFrom(source);
}

void EpsilonUDPConfiguration::_copyFrom(EpsilonLinkConfiguration *source)
{
    auto *usource = qobject_cast<EpsilonUDPConfiguration *>(source);
    if (usource)
    {
        ...
        {
            EpsilonUDPCLient *target = usource->targetHosts()[i];
            if (!contains_target(_targetHosts, target->address, target->port))
            {
                EpsilonUDPCLient *newTarget = new EpsilonUDPCLient(target);
...
void EpsilonUDPConfiguration::_clearTargetHosts()
{
 ...
 */
void EpsilonUDPConfiguration::addHost(const QString host)
{
    // Handle x.x.x.x:p
    if (host.contains(":"))
    {
...
void EpsilonUDPConfiguration::addHost(const QString &host, quint16 port)
{
    ...
    else
    {
        ...
        {
            EpsilonUDPCLient *newTarget = new EpsilonUDPCLient(address, port);
            _targetHosts.append(newTarget);
            _updateHostList();
        }
    }
...
void EpsilonUDPConfiguration::removeHost(const QString host)
{
    if (host.contains(":"))
    {
        ...
        {
            EpsilonUDPCLient *target = _targetHosts.at(i);
            if (target->address == address && target->port == port)
            {
...
void EpsilonUDPConfiguration::setLocalPort(quint16 port)
{
    _localPort = port;
}

void EpsilonUDPConfiguration::saveSettings(QSettings &settings, const QString &root)
{
...
void EpsilonUDPConfiguration::loadSettings(QSettings &settings, const QString &root)
{
...
void EpsilonUDPConfiguration::_updateHostList()
{
    _hostList.clear();
    for (int i = 0; i < _targetHosts.size(); i++)
    {
