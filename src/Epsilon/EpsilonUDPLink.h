...
class EpsilonUDPCLient
{
  public:
    EpsilonUDPCLient(const QHostAddress &address_, quint16 port_) : address(address_), port(port_)
    {
    }
    EpsilonUDPCLient(const EpsilonUDPCLient *other) : address(other->address), port(other->port)
    {
    }
...
class EpsilonUDPConfiguration : public EpsilonLinkConfiguration
{
  ...
  public:
    ...
    EpsilonUDPConfiguration(const QString &name);
    EpsilonUDPConfiguration(EpsilonUDPConfiguration *source);
    ~EpsilonUDPConfiguration();
    ...
    const QList<EpsilonUDPCLient *> targetHosts(void)
    {
        return _targetHosts;
    }
    ...
    {
        return EpsilonLinkConfiguration::TypeUdp;
    }
  ...
  private:
    ...
    void _copyFrom(EpsilonLinkConfiguration *source);

    QList<EpsilonUDPCLient *> _targetHosts;
    QStringList _hostList;
    quint16 _localPort;
};

class EpsilonUDPLink : public EpsilonLinkInterface
{
    Q_OBJECT

  public:
    EpsilonUDPLink(SharedEpsilonLinkConfigurationPtr &config);
    virtual ~EpsilonUDPLink();
  ...
  private:
    ...
    void _writeDataGram(const QByteArray data, const EpsilonUDPCLient *target);

    bool _running;
    QUdpSocket *_socket;
    EpsilonUDPConfiguration *_udpConfig;
    bool _connectState;
    QList<EpsilonUDPCLient *> _sessionTargets;
