...
QGCApplication::QGCApplication(int &argc, char *argv[], bool unitTesting) : QApplication(argc, argv), _runningUnitTests(unitTesting)
{
    _app = this;
    ...
    if (!_runningUnitTests)
    {
        ...
        if (permFile.open(QIODevice::ReadOnly))
        {
            while (!permFile.atEnd())
            {
                ...
                {
                    permFile.close();
                    _exitWithError(QString(tr("The current user does not have the correct permissions to access serial devices. "
    ...
    QString COLUGO_VER{"Colugo 1.0.5:"};
    this->setApplicationVersion(COLUGO_VER + ORIGINAL_VER);
...
void QGCApplication::setLanguage()
{
    ...
    qDebug() << "System reported locale:" << _locale << "; Name" << _locale.name()
             << "; Preffered (used in maps): " << (QLocale::system().uiLanguages().length() > 0 ? QLocale::system().uiLanguages()[0] : "None");
...
bool QGCApplication::_initForNormalAppBoot()
{
    ...
    toolbox()->commtactLinkManager()->loadLinkConfigurationList();
...
void QGCApplication::saveTelemetryLogOnMainThread(QString tempLogfile)
{
    ...
    if (_checkTelemetrySavePath(true /* useMessageBox */))
    {
        ...
        QString saveFileName =
            nameFormat.arg(QDateTime::currentDateTime().toString(dtFormat)).arg(QStringLiteral("")).arg(toolbox()->settingsManager()->appSettings()->telemetryFileExtension);
...
void QGCApplication::_missingParamsDisplay(void)
{
    if (_missingParams.count())
    {
        ...
        showAppMessage(tr("Parameters are missing from firmware. You may be running a version of firmware which is not fully supported or your firmware has a bug in it. Missing params: %1")
                           .arg(params));
    }
}
    ...
    if (errorMsg.isEmpty())
    {
        ...
        if (versionFile.open(QIODevice::ReadOnly))
        {
            ...
            if (_parseVersionText(version, majorVersion, minorVersion, buildVersion))
            {
                ...
                {
                    showAppMessage(tr("There is a newer version of %1 available. You can download it from %2.").arg(applicationName()).arg(toolbox()->corePlugin()->stableDownloadLocation()),
